...
#include "sampled_fentry_10.skel.h"
...
{
	struct sampled_fentry_10_bpf *skel;
	int err;

	char* type = "sampled_fentry_10";
	if(argc > 1){
		type = argv[1];
		printf("Type: %s\n", type);
	}
	...
	skel = sampled_fentry_10_bpf__open_and_load();
	if (!skel) {
	...
	err = sampled_fentry_10_bpf__attach(skel);
	if (err) {
...
cleanup:
	sampled_fentry_10_bpf__destroy(skel);
	return -err;
}
