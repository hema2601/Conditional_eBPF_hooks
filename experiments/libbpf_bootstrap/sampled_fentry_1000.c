...
#include "sampled_fentry_1000.skel.h"
...
{
	struct sampled_fentry_1000_bpf *skel;
	int err;

	char* type = "sampled_fentry_1000";
	if(argc > 1){
		type = argv[1];
		printf("Type: %s\n", type);
	}
	...
	skel = sampled_fentry_1000_bpf__open_and_load();
	if (!skel) {
	...
	err = sampled_fentry_1000_bpf__attach(skel);
	if (err) {
...
cleanup:
	sampled_fentry_1000_bpf__destroy(skel);
	return -err;
}
